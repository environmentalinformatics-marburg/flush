#!/bin/bash
# This script creates a 7 day long rotating backup of the
# SOURCEPATH directory to the BACKUPPATH directory.
# Crontab entry: 0 2 * * * bash /home/dogbert/administration/backup/ei_backup_sd19006_data_exploratories.bash >> /home/dogbert/administration/backup/log_error_sd19006_data_exploratories.log 2>&1 &

SOURCEPATH='/media/memory01/data/exploratories/' 
BACKUPPATH='/mnt/sd19003/sd19006_backup_data_exploratories/'
BACKUPNAME='sd19006_backup_data_exploratories'
...
if ping -qc 20 192.168.191.180 >/dev/null; then
 	if mount | grep /mnt/sd19003 > /dev/null; then
		...
		if less  ${LOGPATH}log_rsync_sd19006_backup_data_exploratories.log | grep error ; then 
				java -jar ${LOGPATH}sender.jar spaska.forteva@geo.uni-marburg.de forteva@staff.uni-marburg.de smtp.staff.uni-marburg.de Backup "Backup sd19006_backup_data_exploratories: Error messages" 
		fi
	else 
