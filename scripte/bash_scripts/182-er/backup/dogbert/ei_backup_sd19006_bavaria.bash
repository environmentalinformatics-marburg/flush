...
# Crontab entry: 30 0 * * * bash /home/dogbert/administration/backup/ei_backup_sd19006_bavaria.bash >> /home/dogbert/administration/backup/log_error_sd19006_bavaria.log 2>&1 &

SOURCEPATH='/media/memory01/ei_data_bavaria/' 
BACKUPPATH='/mnt/sd19003/sd19006_backup_bavaria/'
BACKUPNAME='sd19006_backup_bavaria'
...
if ping -qc 20 192.168.191.180 >/dev/null; then
	if mount | grep /mnt/sd19003 > /dev/null; then
		...
		if less  ${LOGPATH}log_rsync_sd19006_backup_bavaria.log | grep error ; then 
				java -jar ${LOGPATH}sender.jar spaska.forteva@geo.uni-marburg.de forteva@staff.uni-marburg.de smtp.staff.uni-marburg.de Backup "Backup sd19006_backup_bavaria: Error messages" 
	fi
	else 
