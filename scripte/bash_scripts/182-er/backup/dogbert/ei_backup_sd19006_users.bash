...
# Crontab entry: 0 0 * * * bash /home/dogbert/administration/backup/ei_backup_sd19006_users.bash >> /home/dogbert/administration/backup/log_error_sd19006_users.log 2>&1 &

SOURCEPATH='/media/memory01/data/users/' 
BACKUPPATH='/mnt/sd19003/sd19006_backup_users/'
BACKUPNAME='sd19006_backup_users'
...
if ping -qc 20 192.168.191.180 >/dev/null; then
	if mount | grep /mnt/sd19003 > /dev/null; then
		...
		/usr/bin/rsync -a -e --copy-links --copy-unsafe-links --delete --exclude=".*" --exclude="/media/memory01/data/users/fdetsch/R-Server/tmp" --link-dest=${BACKUPPATH}backup_${BACKUPNAME}.1 --log-file=${LOGPATH}log_rsync_${BACKUPNAME}.log ${SOURCEPATH} ${BACKUPPATH}backup_${BACKUPNAME}.0/
		...
		if less  ${LOGPATH}log_rsync_sd19006_backup_kilimanjaro.log | grep error ; then 
				java -jar ${LOGPATH}sender.jar spaska.forteva@geo.uni-marburg.de forteva@staff.uni-marburg.de smtp.staff.uni-marburg.de Backup "Backup sd19006_backup_users: Error messages" 
		fi
	else 
