...
SOURCEPATH='/media/memory01/ei_data_myanmar/' 
...
if ping -qc 20 192.168.191.180 >/dev/null; then
	if mount | grep /mnt/sd19003 > /dev/null; then
		...
		if less  ${LOGPATH}log_rsync_sd19006_backup_myanmar.log | grep error ; then 
				java -jar ${LOGPATH}sender.jar spaska.forteva@geo.uni-marburg.de forteva@staff.uni-marburg.de smtp.staff.uni-marburg.de Backup "Backup sd19006_backup_myanmar: Error messages" 
		fi
	else 
