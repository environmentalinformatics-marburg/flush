...
public class TransferLocal{
	...
	public void createNewFile( String param,  ConfigTransverLocal conf )
	{
		...
		SimpleDateFormat dfNew = new SimpleDateFormat( "yyyy-MM-dd_HHmmss" );
		...
		try{
			...
			try{
				...
				while ( ( line = br.readLine() ) != null ) {
					...
					endDate= line;
					if ( index == 3 ){
						startDate = dfNew.format( df.parse( ( line.split( ";" ) )[0] ) ); 	        	 
					}
				}
				endDate = dfNew.format( df.parse( ( endDate.split( ";" ) )[0] ) ); 
			}
			...
			fos = new FileOutputStream(  conf.get( "scrPath")  + name + "/csv/" + name + "_" +  startDate + "_" + endDate + ".csv", true );
			...
			for ( int i = 0; i < headerRowsNumber; i++ ){
				...
				if (i == 0) {
					String text = "startTime;" + startDate + " ;;;;;;;;;;;;;;;;;;;;;;;;";
					row = text.getBytes();
				}
			...
			fis.close();
			fos.close();
			File fileToMove = new File(conf.get( "scrPath")   + name + "/csv/" + name + "_" +  startDate + "_" + endDate + ".csv");
			File fileToDelete = new File(scrPath);
			if ( fileToMove.length() - fileToDelete.length() > 4 ) {
				File dest = new File(conf.get( "destPath" ) + stationName + "/" + fileToMove.getName());
				boolean isMoved = fileToMove.renameTo(dest);
			    if( isMoved ) {
			    	fileToDelete.delete();
			    }
			}
