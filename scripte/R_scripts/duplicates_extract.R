...
## Set folders
zip.folder <- "/media/dogbert/dev/be/source/"
corrected.folder <- "/media/dogbert/dev/be/corrected/"
...
foreach(c = list.fls, .packages = lib) %dopar% {
  ...
  if (nrow(to_check)!=nrow(to_check.unique)) {## only replace changed files
    file.rename(c, paste0(c, ".duplicate"))
...
list.corrected <- list.files(path = zip.folder,
                             pattern = "txt.duplicate|dat.duplicate",
                             full.names = TRUE,
                             recursive = TRUE)
list.corrected
